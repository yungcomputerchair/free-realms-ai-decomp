...
/* Setting prototype: int CwDeck_loadTextFile(char * path, void * logContext) */

int CwDeck_loadTextFile(char *path,void *logContext)

{
  ...
  int local_4;
                    ...
                    /* Opens a deck text file and parses 'Deck ID:', 'Deck Name:', and 'Card ...'
                       lines, skipping '#' comments and logging parsed IDs/instances. Evidence is
                       the ..\common\rules\property\cwdeck.cpp file string plus deck/card-specific
                       literals and assertions around card id delimiters. */
  local_4 = 0xffffffff;
  puStack_8 = &LAB_01691beb;
  ...
  local_ac = file;
  if (file == (FILE *)0x0) {
    Game_logGeneral(logContext,"Couldn\'t open text file: %s\n",path);
  ...
  do {
    ...
    if (iVar2 == 0) {
      ...
      if (*(char *)pppppppcVar6 == '#') {
        Game_logGeneral(logContext,"discarding line, is a comment\n",value);
      }
      else {
        ...
        if (iVar2 != -1) {
          ...
          iVar2 = FUN_01231320(pppppppuVar3,0,local_50);
          if (iVar2 == 0) {
            Game_logGeneral(logContext,"got deckID!\n",value);
            ...
            if ((uint)piVar1[7] < 0x10) {
              Game_logGeneral(logContext,"deckID set to: %s\n",piVar1 + 2);
            }
            else {
              Game_logGeneral(logContext,"deckID set to: %s\n",piVar1[2]);
            }
            goto joined_r0x01409b21;
          }
        }
        ...
        if (iVar2 != -1) {
          ...
          iVar2 = FUN_01231320(pppppppuVar3,0,local_6c);
          if (iVar2 == 0) {
            Game_logGeneral(logContext,"got deckName!\n",value);
            std::basic_string<char,std::char_traits<char>,std::allocator<char>_>::erase
                      (local_9c,0,local_6c);
            std::basic_string<char,std::char_traits<char>,std::allocator<char>_>::assign
                      ((basic_string<char,std::char_traits<char>,std::allocator<char>_> *)
            ...
            if ((uint)piVar1[0x15] < 0x10) {
              Game_logGeneral(logContext,"deckName set to: %s\n",piVar1 + 0x10);
            }
            else {
              Game_logGeneral(logContext,"deckName set to: %s\n",piVar1[0x10]);
            }
        ...
        if (iVar2 != -1) {
          ...
          iVar2 = FUN_01231320(pppppppuVar3,0,local_18);
          if (iVar2 == 0) {
            Game_logGeneral(logContext,"got Card!\n",value);
            auStack_b0[0] = 0x28;
            iVar2 = FUN_01231320(auStack_b0,0,1);
            if (iVar2 == -1) {
              FUN_012f5a60("idpos != std::string::npos","..\\common\\rules\\property\\cwdeck.cpp",
                           0x244);
            }
            std::basic_string<char,std::char_traits<char>,std::allocator<char>_>::erase
                      (local_9c,0,iVar2 + 1);
            ...
            Game_logGeneral(logContext,"got ID = %d (%s)\n",lVar5);
            pppppppcVar6 = apppppppcStack_44[0];
            if (uStack_30 < 0x10) {
              pppppppcVar6 = (char *******)apppppppcStack_44;
            }
            lVar5 = _atol((char *)pppppppcVar6);
            Game_logGeneral(logContext,"got Instance = %d (%s)\n",lVar5);
            pppppppcVar6 = local_98[0];
            if (local_84 < 0x10) {
              pppppppcVar6 = (char *******)local_98;
            }
