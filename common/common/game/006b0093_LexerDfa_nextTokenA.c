...
undefined * __fastcall LexerDfa_nextTokenA(int param_1)

{
  ...
  uint local_8;
                    ...
                    /* Runs an alternate generated DFA lexer and returns a static token result,
                       tracking input position, line/column, accept states, and lexer state stack
                       transitions. It is structurally the same token loop as LexerDfa_nextToken but
                       uses the 006a helper set and static result storage. */
  if ((_DAT_01bdf204 & 1) == 0) {
    _DAT_01bdf204 = _DAT_01bdf204 | 1;
    _DAT_01bdf1f4 = 1;
    _DAT_01bdf1f8 = &DAT_01bc7e28;
    _atexit(FUN_01746863);
  }
  while( true ) {
    FUN_006aa622();
