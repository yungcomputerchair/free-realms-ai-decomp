// addr: 0x014fd9e0
// name: CommandObject_SendCardIntroductions_ctor
// subsystem: common/common/rules
// Decompiled pseudo-C from FreeRealms.exe (Ghidra). First-party.


undefined4 * __thiscall
CommandObject_SendCardIntroductions_ctor(undefined4 *param_1,undefined4 param_2,void *param_3)

{
  uint uVar1;
  void *pvVar2;
  undefined4 uVar3;
  void *local_c;
  undefined1 *puStack_8;
  int local_4;
  
                    /* Constructs CommandObject_SendCardIntroductions with base CommandObject setup
                       and return values. */
  local_4 = 0xffffffff;
  puStack_8 = &LAB_016b3b83;
  local_c = ExceptionList;
  uVar1 = DAT_01b839d8 ^ (uint)&stack0xffffffe8;
  ExceptionList = &local_c;
  CommandObject_ctor(param_1,param_3);
  local_4 = 0;
  *param_1 = CommandObject_SendCardIntroductions::vftable;
  param_1[2] = CommandObject_SendCardIntroductions::vftable;
  std::basic_string<char,std::char_traits<char>,std::allocator<char>_>::assign
            ((basic_string<char,std::char_traits<char>,std::allocator<char>_> *)(param_1 + 0x12),
             "CommandObject_SendCardIntroductions",0x23);
  *(undefined1 *)(param_1 + 0x46) = 1;
  *(undefined1 *)((int)param_1 + 0x81) = 0;
  param_1[0x47] = 0x174;
  pvVar2 = EvaluationEnvironment_getOriginCard(param_3);
  param_1[0x19] = pvVar2;
  pvVar2 = Mem_Alloc(0x18);
  local_4._0_1_ = 1;
  if (pvVar2 == (void *)0x0) {
    uVar3 = 0;
  }
  else {
    uVar3 = ReturnValueMap_ctor(uVar1);
  }
  local_4 = (uint)local_4._1_3_ << 8;
  param_1[0x1c] = uVar3;
  EvaluationEnvironment_getMember14(param_3);
  uVar3 = GamePlayer_getPlayerId();
  ReturnValueMap_setInteger(4,uVar3);
  FUN_014181b0(0x16,param_2);
  ExceptionList = local_c;
  return param_1;
}

